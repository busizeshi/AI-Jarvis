-- =====================================================================
-- 拾笺 NoteGather · 第一阶段数据库初始化脚本（MySQL 8.0）
-- 说明：
--   1. 目标数据库：MySQL 8.0（InnoDB 引擎，utf8mb4 / utf8mb4_unicode_ci）。
--   2. 主键统一使用雪花算法（Snowflake）由应用层生成，类型 BIGINT，不设自增。
--   3. 公共字段与 common-mybatis BaseEntity 对齐：
--      create_time / update_time / deleted（0=未删除，1=已删除）。
--      update_time 使用 ON UPDATE CURRENT_TIMESTAMP(3) 自动维护。
--   4. “活跃唯一”约束借助生成列（未删除时取业务值、删除后置 NULL）+ 唯一索引实现。
--   5. 由 docker-compose 挂载至 /docker-entrypoint-initdb.d 首次启动自动执行。
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS t_parse_task (
    ...
    ) VIRTUAL COMMENT '活跃解析状态：未删除且进行中时=status，否则=NULL，用于防并发唯一约束',
    PRIMARY KEY (id),
